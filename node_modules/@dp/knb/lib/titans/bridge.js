...
var DPApp = {
  ...
  log: function () {
    ...
    if (this._isDebug || isForceDebug()) return setTimeout(() => alert(message.join(' ')))
  ...
  enqueue: function (method, args, callback) {
    var self = this
    this.queue = this.queue || queue(({method, args, callback}) => self.doSendMessage(method, args, callback))
    this.log('[Enqueue]', method)
    this.queue.push({method, args, callback})
    // run task every 1000ms if queue is not empty
    this._dequeueTimeout = setTimeout(() => self.dequeue('Auto'), self._delay * self.queue.tasks.length)
  },
  ...
  callback: function (callbackId, retValue, isFromMTNB) {
    ...
    setTimeout(() => callback.call(self, retValue), 0)
