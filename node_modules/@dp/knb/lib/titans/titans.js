...
var Titans = {
  ...
  setBridge: bridge => Bridge.setBridge(bridge),
  getBizName: function () {return this._cfg.bizname},
  ready: callback => Bridge.ready(callback),
  log: Bridge.log.bind(Bridge),
  loadAPIs: function ({ type='base', apis=[] }) {
    var self = this
    ...
    apis
      ...
      .forEach(API => {
        ...
        Object.keys(API.statics || {}).forEach(key => self[name][key] = API.statics[key])
      })
  },
  // 检查一个nativeApi是否存在
  ...
  // 检查一个KNB方法是否存在
  isApiSupported: function ({apiName, success=noop}) {
