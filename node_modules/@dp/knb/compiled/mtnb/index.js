(function() {
 ...
 function getVersionInfo(name) {
   try {
     switch (name) {
       ...
       case "KNB":
         ...
         return navigator.userAgent.match(/KNB\/(\S+)/g).map(function (str) {
  ...
  var canDetectInternalAPI = (mtnbNativeVersions == '0' ? [] : mtnbNativeVersions).some(function (version) { return parseFloat(version) >= 1.1; });
  ...
  [{
    ...
    var callbackMapper = ref.callbackMapper;

    sentinel.increase();
    isInternalSupported((businessName + "." + moduleName + "." + methodName)).then(function (supported) {
      sentinel.decrease();
      if (!supported) return fallbackRegFn();
      MTNB.reg({name: name, businessName: businessName, moduleName: moduleName, methodName: methodName, callbackMapper: callbackMapper});
    });
  });
