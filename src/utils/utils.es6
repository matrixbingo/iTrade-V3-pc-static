...
export let HipoUtil = HipoUtil || {};

export function mergeCheck(arr1, arr2) {
    try{
        for (var key1 in arr1) {
            for(var key2 in arr2){
                if(key1 == key2){
                    console.error('存在两个相同的key:', key1, arr1, arr2);
                    //alert('存在两个相同的key:' + key1);
                }
            }
        }
    }catch (e){
        console.error("mergeCheck : " ,e);
    }
}
...
export function merge(arr) {
    let rs = {};
    for (var i in arr) {
        this.mergeCheck(arr[i].param, rs);
...
HipoUtil.hippo = function (module, action, title, source) {
    try {
        if (_hip) {
            //console.log('userid', userid);
            _hip.push(['mv', {module: module, action: action, title: title, source: source}]);
        }
    } catch (e) {
        console.error('选择全部，hippo打点失败！', e);
    }
}

HipoUtil.hippoByType = function (type) {
    let params = getUrls();
    params.source = params.source?params.source:'';
    HipoUtil.hippo('agent', type, '代理商注册', params.source);
}

HipoUtil.pv = function () {
    HipoUtil.hippoByType('pv');
    HipoUtil.uv();
}

HipoUtil.uv = function () {
    const COOKIE_NAME = 'apollo-agent-pc-static-user-lonin-cookie-time-oneDay';
    const time = Cookies.get(COOKIE_NAME);
    if (!time) {
        Cookies.set(COOKIE_NAME, new Date(), {expires: 1});
        HipoUtil.hippoByType('uv');
    }
}

HipoUtil.submit = function () {
    HipoUtil.hippoByType('submit');
}

FormUtil.share = function(){
    try{
        KNB.configShare({
            title:"美团点评代理商招募",
            desc:"美团点评代理商招募",
            image:"http://img5.imgtn.bdimg.com/it/u=1148759641,3162100664&fm=11&gp=0.jpg",
            url:"https://www.dianping.com/apollo/agent/index?source=share",
            channel: [KNB.share.WECHAT_FRIENDS,KNB.share.WECHAT_TIMELINE,KNB.share.QQ,KNB.share.SMS,KNB.share.QZONE,KNB.share.WEIBO,KNB.share.EMAIL,KNB.share.COPY],
            handle: function(){
                //alert('分享按钮被点击了');
            },
            success: function(channel){
                //alert('分享成功');
                HipoUtil.hippo('agent', 'click', '代理商注册', 'share');
            }
        });
    }catch(e){
        console.error('分享报错' ,e);
    }
}
