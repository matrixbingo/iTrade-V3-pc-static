...
class SelectPlus extends Component {
    ...
    getOptions() {
        ...
        if (DataUtil.is.Object(list)) {
            _.each(list, function (name, id) {
                if(_this.props.addOptions.need){
                    options.push(<option value={_this.props.addOptions.options.id} key={_this.props.addOptions.options.id}>{_this.props.addOptions.options.name}</option>);
                }
